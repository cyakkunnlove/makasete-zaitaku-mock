...
export function buildRegionalAdminInvitationEmail(input: {
  ...
  const subject = '【マカセテ在宅】リージョン管理者アカウント招待'
  const text = [
    `${input.fullName} 様`,
    '',
    'マカセテ在宅への招待をお送りします。',
    `付与予定の立場: リージョン管理者`,
    `対象リージョン: ${input.regionName}`,
    '',
    ...
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;line-height:1.6;color:#111;background:#f7f7f8;padding:24px;">
      <div style="max-width:640px;margin:0 auto;background:#fff;border:1px solid #e5e7eb;border-radius:14px;overflow:hidden;">
        ...
        <div style="padding:24px;">
          ...
          <p><strong>付与予定の立場:</strong> リージョン管理者<br /><strong>対象リージョン:</strong> ${input.regionName}</p>
          ...
          <p style="margin-top:24px;font-size:14px;color:#4b5563;">このメールに心当たりがない場合は、破棄してください。</p>
        </div>
      </div>
    </div>
  `
