...
import { resendAccountInvitation } from '@/lib/account-invitation-service'
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const reauthResponse = await ensureRecentReverification(user, {
    reason: 'account_invitation_resend',
    nextPath: '/dashboard/staff',
  })
  if (reauthResponse) return reauthResponse

  const result = await resendAccountInvitation({ actor: user, request, invitationId: params.id })
  if (!result.ok) {
